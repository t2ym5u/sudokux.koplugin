...
local SudokuXBoardWidget = BaseBoardWidget:extend{
    board = nil,
}

function SudokuXBoardWidget:paintTo(bb, x, y)
    ...
    local diag_color = Blitbuffer.COLOR_GRAY_D
    for r = 1, n do
        bb:paintRect(x + math.floor((r - 1) * cell), y + math.floor((r - 1) * cell),
                     math.ceil(cell), math.ceil(cell), diag_color)
    end
    for r = 1, n do
        local c = n + 1 - r
        bb:paintRect(x + math.floor((c - 1) * cell), y + math.floor((r - 1) * cell),
                     math.ceil(cell), math.ceil(cell), diag_color)
    end
